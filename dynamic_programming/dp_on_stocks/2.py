def rec(arr,i,state): #0 means we are in state to buy
    if i==len(arr):
        return 0
    if state==0:
        we_buy = -arr[i] + rec(arr,i+1,1) #1 means we are in state to sell
        return max(we_buy,rec(arr,i+1,0))
    elif state==1:
        we_sell = arr[i] + rec(arr,i+1,0)
...
def memo(arr,i,state,dp_memo):
    ...
    if state==0:
        ...
        we_sell = arr[i] + memo(arr,i+1,0,dp_memo)
