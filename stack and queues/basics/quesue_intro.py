class Queue:
    ...
    def pop(self) -> int:
        ...
        if self.currSize == 1:
            self.start = -1 #removing the only element which was presesnt, so set the start and end values to -1
            self.end = -1
        else:
            self.start = (self.start + 1) % self.maxSize #in the queues we just pop the element from the first.
        self.currSize -= 1
        return popped
    ...
    def top(self) -> int:
        if self.start == -1:
            ...
            exit(1)
        return self.arr[self.start] #even the top function we just use it to find thee first element
